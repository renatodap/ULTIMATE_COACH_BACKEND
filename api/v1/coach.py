...
def get_coach_service():
    """Get UnifiedCoachService instance."""
    return get_unified_coach_service()
...
async def send_message(
    ...
    **Smart Routing:**
    - Trivial queries: Canned responses (FREE, 0ms)
    - Simple queries: Groq Llama 3.3 70B ($0.01, 500ms)
    - Complex queries: Claude 3.5 Sonnet ($0.15, 2000ms)

    **Auto-classification:**
    - CHAT: Questions, advice requests → AI response
    - LOG: Meals, workouts, measurements → Log preview + AI response

    **Features:**
    - Auto language detection (EN, PT, ES)
    - Agentic tool calling (on-demand data fetching)
    - Conversation memory (recent context)
    - Quick entry logging (detected logs)

    **Response Time:**
    - Canned: <50ms
    - Simple: 500-800ms (Groq)
    - Complex: 1500-2500ms (Claude)

    **Cost:**
    - Canned: $0.00
    - Simple: $0.01
    - Complex: $0.10-0.15
    - Average: $0.035 (with smart routing)
    """
    try:
        ...
        logger.info(
            f"[CoachAPI] ✅ Response ready: {result['model_used']}, "
            f"{response_time_ms}ms, ${result['cost_usd']:.4f}"
        )
        ...
        return MessageResponse(
            ...
            quick_entry_id=result.get("quick_entry_id"),
            should_show_preview=result.get("should_show_preview", False),
